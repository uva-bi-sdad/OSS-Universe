...
###Data Source: 
All copyrights filed through the U.S. Copyright Office with the U.S. Library of Congress are publicly available via an online catalog. These files are stored in HTML format with and can be accessed at a maximum of 10,000 records per search. There is currently no bulk download functionality for these files, so we developed a web-scraping strategy to access the files. We used the following approach to guide our progress: 
...
We refined our method by modifying detection rules to increase matching records and avoid overlaps between sectors. Our result shows 81.06% of the 39,969 unique claimant names has been identified as in either of the five sectors.
...
The goal of this project is to identify OSI projects with copyright application. In attempting to find such OSI projects within the Trademarks over the past 19 years, I performed a number of queries on the database. Each of these are keywords associated with Open source software.

1. GNU (projects often associated with OSI), returned 34 results, mostly from the Free Software Foundation 2002-2004.
2. GPI (General Public Lisence), returned 8 results, 7 of which seemed to be related to publically available files.
3. MIT, APACHE (common open source liscenses) both have several hundred results with no clear indicators of public liscence.
4. BSD (another open source liscence), returned 13 results, with 5 reasonably being a BSD open source license.
5. Public, 1240 results with no clear OSI indicators, though few if any seem OSI
6. “Open Source” returns 124 rows, but many are simply reporting that they used open source, some did not (9 had OS in the title)
7. R = 8, python = 1, android =6 (popular Open source platforms), none look like OSI

These searches return a tiny subset of copyrights, totaling to ~55 total OSI files that were filed with the copyright office over the 15 year period of interest. 

============================= CHALLENGES ============================= 

The team faced three major challenges when attempting to gather and analyze the data. 
First, searching for OSS software within the data yielded very few results, as most claimants did not explicitly include “open source software,” “OSS,” or a variation of this phrase in the copyright title. Wihtout consistent listings of OSS, the current database fails to provide a complete overview of innovation with OSS.
